...
public class AuctionManageService {
    ...
    public boolean createAuction(AuctionDTO auctionDTO) {
        Request<AuctionDTO> request = new Request<>(Action.CREATE_AUCTION, auctionDTO);
        SocketClient.getInstance().send(request);
        Response response = SocketClient.getInstance().receive();

        if (response != null && response.getStatus().toString().equals("SUCCESS")) {
            return true;
        } else {
            return false;
        }
    ...
    public List<AuctionDTO> getAuctionsBySellerId(int sellerId) {
        Request<Integer> request = new Request<>(Action.GET_SELLER_ITEMS, sellerId);
        SocketClient.getInstance().send(request);
    ...
    public boolean deleteAuction(AuctionDTO auctionDTO) {
        ...
        Request<Integer> request = new Request<>(Action.DELETE_AUCTION, auctionDTO.getId());
        SocketClient.getInstance().send(request);
    ...
    public boolean editAuction(AuctionDTO auctionDTO) {
        ...
        Request<AuctionDTO> request = new Request<>(Action.EDIT_AUCTION, auctionDTO);
        SocketClient.getInstance().send(request);
